...
\begin{itemize}
	...
	\item A trained and evaluated CycleGAN baseline (Chapter~5) for the Stage~2 view-translation problem, including a specific, evidence-based diagnosis of why its current FID score (269.9) falls well short of the target ($\leq$\,55): a discriminator collapse traced to the epoch level using the training logs, not a vague appeal to ``GANs are unstable''.
	\item An in-progress CTX/HiRISE co-registration pipeline for the Stage~1 super-resolution corpus, with a working smoke test on five image pairs.
\end{itemize}

\section{Overview of Dissertation}

Chapter~2 reviews the literature underpinning each stage of the proposed pipeline and identifies the specific gap this work fills. Chapter~3 presents the methodology: the mathematical formulation of the three-stage architecture, the dataset design, and the evaluation framework. Chapter~4 describes what has actually been implemented so far --- the data acquisition and preprocessing pipelines for both the HiRISE and rover corpora, including the real-resolution corpus rebuild, and the CycleGAN model and training infrastructure built to establish a Stage~2 baseline. Chapter~5 presents the results of training that baseline: the loss behaviour over 50 epochs, the FID evaluation, representative sample translations, and a diagnosis of the specific failure mode observed, together with candidate fixes. Resolving that diagnosis, building the physics-constrained Neural Schr\"{o}dinger Bridge, completing the Stage~1 super-resolution corpus, and implementing and evaluating the Gaussian Splatting rendering stage remain as future work. This draft does not yet include a final Results/Evaluation chapter for the completed system or a Conclusions chapter, since that work is still in progress.
